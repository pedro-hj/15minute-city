...
def load_osm_graph(
    ...
    # RECOVERY POINT: Check if city bounds already exist in DB
    region_gdf = None
    if pipeline:
        try:
            region_gdf = pipeline.get_city_boundary(
                region["city"], region.get("country", "Brazil")
            )
        except Exception as e:  # noqa: BLE001
            logger.debug("Database city boundary recovery skipped: %s", e)
            region_gdf = None

    # If the cache/db does not exist, the raw .pbf file is cropped according to the region being analyzed
    if region_gdf is None:
        region_gdf = ox.geocode_to_gdf(region)

        # PERSISTENCE POINT: Save city boundary to 'city' table
        if pipeline:
            try:
                pipeline.save_city(
                    region["city"], region.get("country", "Brazil"), region_gdf
                )
            except Exception as e:  # noqa: BLE001
                logger.debug("Database city persistence skipped: %s", e)
    ...
    # PERSISTENCE POINT: "G" -> "node" TABLE
    if pipeline and execution_id:
        try:
            pipeline.save_graph_nodes(execution_id, G)
        except Exception as e:  # noqa: BLE001
            logger.debug("Database graph nodes persistence skipped: %s", e)
...
def load_services_geojson(
    ...
    # RECOVERY POINT: "service" TABLE -> "services_geojson"
    ...
    services_geojson = services_geojson[
        services_geojson["service_type"].isin(services_filter)
    ]
    ...
    for service_type, sub_service in services_geojson.groupby("service_type"):
        result[service_type] = dict(
            zip(sub_service.get("name"), sub_service.get("geometry"))
        )

    location_services, data_to_db = organizes_data(G, result)

    # PERSISTENCE POINT: "data" -> "service" TABLE
    if pipeline and execution_id:
        try:
            pipeline.save_services_from_organizer(execution_id, data_to_db)
        except Exception as e:  # noqa: BLE001
            logger.debug("Database services persistence skipped: %s", e)

    return location_services


def organizes_data(G: nx.MultiDiGraph, result: dict) -> tuple[dict, dict]:
    ...
    for tag, services in result.items():
        ...
        for service, coordinate in services.items():
            pair_coordinates = np.array([[coordinate.x, coordinate.y]])
            _distances, id = tree.query(pair_coordinates, k=1)
            node_id = next(node_ids[idx] for idx in id)
            node_data = [service, node_id, coordinate]
            points.append(node_id)
            data.append(node_data)
        location_services[tag] = points
        data_to_db[tag] = data
    return location_services, data_to_db
...
def create_walking_graph(
    ...
    vias = vias[
        vias.geometry.notna() & ~vias.geometry.is_empty & vias.geometry.is_valid
    ].copy()

    vias = vias[vias.geometry.geom_type == "LineString"].copy()

    if vias.empty:
        raise ValueError("Nenhuma geometria LineString válida foi encontrada.")
    ...
    if metric_crs is None:
        raise ValueError("Não foi possível estimar automaticamente um CRS métrico.")
    ...
    for index in range(len(vias_metricas)):
        ...
        for position in range(len(metric_coords) - 1):
            ...
            segment_metric = LineString([metric_start, metric_end])

            segment_wgs84 = LineString([geographic_start, geographic_end])
